...
export default function DashboardPage() {
  ...
  const categoryCounts = courses.reduce<Record<string, number>>((acc, c) => {
    ...
    acc[cat] = (acc[cat] || 0) + (c.studentsEnrolled || 1);
    return acc;
  }, {});
      ...
      <div className="flex-1 p-6 space-y-6 overflow-y-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard 
            ...
            subtitle="Active bootcamps"    
            icon={BookOpen}    
            color="blue"   
            trend={{ value: "Live from API", up: true }} 
          />
          <StatCard 
            title="Applications"   
            value={totalApps}             
            ...
            icon={FileText}    
            color="orange" 
            trend={{ value: `${pending} pending`, up: true }} 
          />
          <StatCard 
            title="Approved"       
            value={approved}              
            ...
            icon={CheckCircle} 
            color="green"  
            trend={{ value: `${approvalRate}% rate`, up: approvalRate > 50 }} 
          />
          <StatCard 
            title="Scholarships"   
            value={scholarships}                     
                ...
                {Object.entries(categoryCounts).map(([cat, count]) => (
                  <div key={cat}>
                    <div className="flex justify-between text-xs mb-1">
                      <span className={`px-2 py-0.5 rounded-full font-medium ${categoryColors[cat] ?? "bg-gray-100 text-gray-600"}`}>
                        {cat}
                      </span>
                  ...
                  <div key={c.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-gray-400">
                        {c.studentsEnrolled || 0} students · ⭐ {c.rating || "—"}
                      </p>
                    </div>
                  </div>
                ))}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            ...
            <a href={`/${typeof window !== "undefined" ? window.location.pathname.split("/")[1] : "en"}/admin/applications`} 
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {recentApps.map((app, i) => {
                    ...
                    const course = app.courseId?.title || app.courseId || app.course || "—";
